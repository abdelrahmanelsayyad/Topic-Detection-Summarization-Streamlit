...
def analyze(text: str, topic_threshold=0.2, max_topics=3, summary_min_length=15, summary_max_length=45):
    ...
    with st.spinner("😀 Analyzing sentiment..."):
        try:
            ...
            # DistilBERT has max token limit of 512
            words = text.split()
            
            # Limit to ~450 words to ensure we stay under the 512 token limit
            # (tokens are usually more than words due to subword tokenization)
            if len(words) > 450:
                # Use only first 450 words for sentiment analysis
                text_for_sentiment = " ".join(words[:450])
                st.info("Text was truncated to fit within token limits for sentiment analysis.")
            else:
                text_for_sentiment = text
            
            # Try a much shorter version if still failing
            try:
                sentiment = sentiment_pipe(text_for_sentiment)[0]
            except Exception:
                # If it still fails, try with an even shorter text
                text_for_sentiment = " ".join(words[:250])
                sentiment = sentiment_pipe(text_for_sentiment)[0]
            ...
            st.warning(f"Sentiment analysis encountered an issue: {str(e)}")
            # Fallback sentiment
            results["sentiment"] = {
                "label": "NEUTRAL",  
                "score": 50
    ...
    with st.spinner("📝 Generating summary..."):
        try:
            ...
            # Bart models typically have a limit of 1024 tokens
            if input_length > 900:  # Conservative limit to account for tokenization
                # Split text and summarize first part
                text_for_summary = " ".join(text.split()[:900])
                st.info("Text was truncated to fit within token limits for summarization.")
                try:
                    summary = sum_pipe(
                        text_for_summary, 
                    ...
                    # If it still fails, try with an even shorter text
                    text_for_summary = " ".join(text.split()[:700])
                    summary = sum_pipe(
                        text_for_summary, 
                        max_length=adjusted_max_length, 
                        min_length=min(summary_min_length, adjusted_max_length - 5), 
                ...
                # Generate summary with normal text
                summary = sum_pipe(
                    text, 
                    max_length=adjusted_max_length, 
                    min_length=min(summary_min_length, adjusted_max_length - 5), 
            ...
            st.warning(f"Summary generation encountered an issue: {str(e)}")
            # Fallback summary
            results["summary"] = "Could not generate summary due to an error."
...
def bot_reply(user_text: str, topic_threshold=0.2, max_topics=3, summary_min_length=15, summary_max_length=45):
    ...
    try:
        ...
        # Catch any other exceptions that might occur during analysis
        error_msg = f"⚠️ I encountered an error while analyzing your text: {str(e)}"
        st.error(error_msg)
